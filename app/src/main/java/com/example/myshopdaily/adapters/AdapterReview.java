...
public class AdapterReview extends RecyclerView.Adapter<AdapterReview.HolderReview>{
    ...
    public void onBindViewHolder(@NonNull HolderReview holder, int position) {
        ...
        //we also need Info (profile img, name) of user who wrote the review: we can do it using uid of user
        ...
        ref.child(uid)
                .addValueEventListener(new ValueEventListener() {
                    ...
                    public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                        ...
                        try{
                            Picasso.get().load(profileImage).placeholder(R.drawable.ic_person_grey).into(holder.profileTv);
                        }
                        catch (Exception e){
                            //if anything goes wrong setting image (exception occurs), set default image
                            holder.profileTv.setImageResource(R.drawable.ic_store_grey);
    ...
    //view holder class, holds/inits views recycleview
